...
const rawAccountId =
  option('--account-id') ?? process.env.PUBLISHER_ACCOUNT_ID ?? null;
...
if (command === undefined || accountId === null) {
    ...
    Effect.gen(function* () {
      if (command === 'profile') {
        ...
        const normalizedHandle =
          handle === null ? null : normalizeProfileHandle(handle);
        if (normalizedHandle === null || displayName === null) {
          return yield* Effect.fail(
            new Error('Missing or invalid profile options.'),
          );
        }
      ...
      if (command === 'publish') {
        ...
        if (boardId === null)
          return yield* Effect.fail(new Error('A valid board id is required.'));
        const publicId = yield* publishing.publish(boardId);
        yield* Effect.sync(() =>
          directory.setBoard(accountId, boardId, publicId),
        );
        console.log(
          JSON.stringify(
            { boardId, publicId, sharePath: `/share/${publicId}` },
            null,
            2,
          ),
        );
      ...
      if (command === 'unpublish') {
        ...
        if (unpublished)
          yield* Effect.sync(() => directory.removeBoard(accountId, boardId));
  ...
  await Effect.runPromise(
    program.pipe(
      Effect.retry(
        Schedule.recurs(6).pipe(
          Schedule.addDelay(() => Effect.succeed(Duration.millis(75))),
        ),
      ),
