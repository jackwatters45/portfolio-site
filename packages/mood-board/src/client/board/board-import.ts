...
const ImportedUnknownRecordSchema = Schema.Record(
  Schema.String,
  Schema.Unknown,
);
...
const ImportedItemGeometrySchema = Schema.Struct({
  x: Schema.Finite.check(
    Schema.isBetween({ minimum: -20_000, maximum: 20_000 }),
  ),
  y: Schema.Finite.check(
    Schema.isBetween({ minimum: -20_000, maximum: 20_000 }),
  ),
...
const ImportedCameraSchema = Schema.Struct({
  x: Schema.Finite.check(
    Schema.isBetween({ minimum: -50_000, maximum: 50_000 }),
  ),
  y: Schema.Finite.check(
    Schema.isBetween({ minimum: -50_000, maximum: 50_000 }),
  ),
...
const decodeImportedUnknownRecord = Schema.decodeUnknownOption(
  ImportedUnknownRecordSchema,
);
const decodeImportedBoardPayload = Schema.decodeUnknownOption(
  ImportedBoardPayloadSchema,
);
const decodeImportedItemGeometry = Schema.decodeUnknownOption(
  ImportedItemGeometrySchema,
);
...
const decodeImageAnnotationTitle = Schema.decodeUnknownOption(
  ImageAnnotationTitleSchema,
);
...
export function normalizeImportedBoard(
  ...
  if (candidate === null)
    throw new Error('That file does not contain a mood board.');
  const rawBoard = Option.getOrNull(
    decodeImportedBoardPayload(candidate.board ?? candidate),
  );
  if (rawBoard === null)
    throw new Error('That mood board format is not supported.');
  ...
  const items: BoardItem[] = rawBoard.items.map((item, index) => {
    ...
    if (entry.mediaId !== undefined) {
      if (!options.allowManagedMedia) {
        throw new Error(
          'Managed media must be imported from a portable .moodboard archive.',
        );
      }
    ...
    if (kind === 'image') {
      ...
      if (entry.src !== undefined) {
        ...
        if (rawSource === undefined)
          throw new Error(`Image ${index + 1} has no source.`);
        const decodedSource = Option.getOrNull(
          decodeSupportedImageSource(rawSource),
        );
        const embedded = /^data:image\/(?:png|jpe?g|webp|gif);base64,/i.test(
          rawSource,
        );
        if (
          decodedSource === null ||
          ...
          throw new Error(
            `Image ${index + 1} uses an unsupported or oversized source.`,
          );
        }
        src = decodedSource;
      }
      ...
      src =
        normalizeSpotifySource(importedString(entry.src) ?? '') ?? undefined;
      if (src === undefined)
        throw new Error(`Spotify card ${index + 1} has an invalid source.`);
    } else if (kind === 'youtube') {
      if (mediaId !== undefined)
        throw new Error(`YouTube card ${index + 1} cannot use managed media.`);
      src =
        normalizeYouTubeSource(importedString(entry.src) ?? '') ?? undefined;
      if (src === undefined)
        throw new Error(`YouTube card ${index + 1} has an invalid source.`);
    } else if (kind === 'x') {
      if (mediaId !== undefined)
        throw new Error(`X card ${index + 1} cannot use managed media.`);
      src = normalizeXPostSource(importedString(entry.src) ?? '') ?? undefined;
      if (src === undefined)
        throw new Error(`X card ${index + 1} has an invalid source.`);
    } else if (kind === 'audio') {
      if ((entry.src === undefined) === (mediaId === undefined)) {
      ...
      if (entry.src !== undefined) {
        src =
          normalizeDirectAudioSource(importedString(entry.src) ?? '') ??
          undefined;
        if (src === undefined)
          throw new Error(
            `Audio card ${index + 1} requires a hosted HTTPS source.`,
          );
      }
    } else if (mediaId !== undefined) {
      throw new Error(
        `Only image and audio items may use managed media (item ${index + 1}).`,
      );
    }
    if (
      kind === 'spotify' &&
      (geometry.width < MIN_AUDIO_CARD_WIDTH ||
        geometry.height < MIN_SPOTIFY_AUDIO_CARD_HEIGHT)
    ) {
    ...
    if (
      kind === 'youtube' &&
      (geometry.width < MIN_AUDIO_CARD_WIDTH ||
        geometry.height < MIN_YOUTUBE_AUDIO_CARD_HEIGHT)
    ) {
    ...
    if (
      kind === 'audio' &&
      (geometry.width < MIN_AUDIO_CARD_WIDTH ||
        geometry.height < MIN_NATIVE_AUDIO_CARD_HEIGHT)
    ) {
    ...
    if (kind === 'x') {
      ...
      xHideThread = Option.getOrUndefined(
        decodeImportedBoolean(entry.xHideThread),
      );
      xAuthorName = Option.getOrUndefined(
        decodeXAuthorName(
          cleanXSnapshotText(entry.xAuthorName, MAX_X_AUTHOR_NAME_CHARACTERS),
        ),
      );
      xPostText = Option.getOrUndefined(
        decodeXPostText(
          cleanXSnapshotText(entry.xPostText, MAX_X_POST_TEXT_CHARACTERS),
        ),
      );
      ...
      if (
        ...
        throw new Error(
          `X card ${index + 1} has invalid settings or snapshot metadata.`,
        );
    } else if (hasXFields) {
      throw new Error(
        `Only X cards may contain X metadata (item ${index + 1}).`,
      );
    }
    ...
    if (kind === 'website') {
      websiteUrl =
        normalizeWebsiteUrl(importedString(entry.websiteUrl) ?? '') ??
        undefined;
      websiteImageUrl =
        entry.websiteImageUrl === undefined
          ? undefined
          : (normalizeWebsiteImageUrl(
              importedString(entry.websiteImageUrl) ?? '',
            ) ?? undefined);
      websiteTitle = Option.getOrUndefined(
        decodeWebsiteTitle(
          importedString(entry.websiteTitle)
            ?.trim()
            .slice(0, MAX_WEBSITE_TITLE_CHARACTERS),
        ),
      );
      ...
      if (
        ...
        throw new Error(
          `Website card ${index + 1} has an invalid preview snapshot.`,
        );
    } else if (hasWebsiteFields) {
      throw new Error(
        `Only website cards may contain website metadata (item ${index + 1}).`,
      );
    }
    ...
    if (
      entry.annotationTitle !== undefined ||
      entry.annotationDescription !== undefined
    ) {
      if (kind !== 'image') {
        throw new Error(
          `Only image items may have annotations (item ${index + 1}).`,
        );
      }
      const rawAnnotationTitle = importedString(entry.annotationTitle);
      if (
        entry.annotationTitle !== undefined &&
        rawAnnotationTitle === undefined
      ) {
        throw new Error(`Image ${index + 1} has an invalid annotation title.`);
      }
      const rawAnnotationDescription = importedString(
        entry.annotationDescription,
      );
      if (
        entry.annotationDescription !== undefined &&
        rawAnnotationDescription === undefined
      ) {
        throw new Error(
          `Image ${index + 1} has an invalid annotation description.`,
        );
      }
    ...
    if (entry.href !== undefined) {
      ...
      if (kind !== 'image' || rawHref === undefined) {
        throw new Error(
          `Only image items may have an external link (item ${index + 1}).`,
        );
      }
    ...
    const text =
      kind === 'note'
        ? Option.getOrUndefined(decodeBoardItemText(entry.text))
        : undefined;
    ...
    const label = allowsLabel
      ? Option.getOrUndefined(
          decodeBoardItemLabel(
            importedString(entry.label)?.slice(
              0,
              MAX_BOARD_ITEM_LABEL_CHARACTERS,
            ),
          ),
    ...
    if (kind === 'swatch') {
      ...
      if (color === undefined) {
        throw new Error(
          `Color ${index + 1} must use six-digit #RRGGBB format.`,
        );
      }
    }

    const rotation =
      Option.getOrNull(decodeImportedFinite(entry.rotation)) ?? 0;
    ...
    return {
      ...
      order: BoardItemOrderSchema.make(
        Math.min(
          MAX_BOARD_ITEM_ORDER,
          Math.max(MIN_BOARD_ITEM_ORDER, Math.trunc(geometry.order)),
        ),
      ),
      src,
      mediaId,
  ...
  if (rawBoard.backgroundMediaId !== undefined) {
    ...
    const decodedBackgroundMediaId = Option.getOrNull(
      decodeMediaId(rawBoard.backgroundMediaId),
    );
  ...
  if (rawBoard.background !== undefined) {
    background =
      normalizeHexColor(importedString(rawBoard.background) ?? '') ?? undefined;
    if (background === undefined) {
      throw new Error(
        'The board background must use six-digit #RRGGBB format.',
      );
    }
  }
  ...
  const title = Option.getOrUndefined(
    decodeBoardTitle(
      importedString(rawBoard.title)?.slice(0, MAX_BOARD_TITLE_CHARACTERS),
    ),
  );
  const board: Board = {
    version: 1,
  ...
  if (candidate.camera) {
    const decodedCamera = Option.getOrNull(
      decodeImportedCamera(candidate.camera),
    );
