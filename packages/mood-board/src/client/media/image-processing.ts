...
export {
  IMAGE_FILE_ACCEPT,
  MAX_HEIC_SOURCE_IMAGE_PIXELS,
  MAX_SOURCE_IMAGE_AXIS,
  MAX_SOURCE_IMAGE_BYTES,
...
export function blobToDataUrl(
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      if (Predicate.isString(reader.result)) resolve(reader.result);
...
const metadataTime = flow(
  Schema.decodeUnknownOption(
    Schema.Union([Schema.Date, Schema.Finite, Schema.String]),
  ),
  Option.map((value) => {
    ...
    void createImageBitmap(file, {
      ...
      .catch(
        flow(
          ...
          Option.getOrElse(() => new Error('That image could not be decoded.')),
          fail,
        ),
      );
  });
...
export async function ingestImageFile(
  ...
  try {
    bitmap = await createNativeBitmap(
      file,
      metadata,
      PREPARED_IMAGE_LONGEST_EDGE,
      signal,
