...
export class PublicApiError extends Schema.Error<PublicApiError>(
  'PublicApiError',
)({
...
const requestJson = Effect.fn('PublicApiClient.requestJson')(function* (
  path: string,
) {
...
const loadPublicProfileEffect = Effect.fn('PublicApiClient.loadPublicProfile')(
  function* (handle: ProfileHandle) {
...
const loadPublicBoardEffect = Effect.fn('PublicApiClient.loadPublicBoard')(
  function* (publicId: PublicId) {
    const value = yield* requestJson(
...
): Promise<PublicProfile> => Effect.runPromise(loadPublicProfileEffect(handle));
