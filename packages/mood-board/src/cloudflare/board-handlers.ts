...
export const CloudflareBoardHandlers = BoardRpcs.toLayer(
  Effect.gen(function* () {
    ...
    const bestEffort = Effect.fn('CloudflareBoardHandlers.bestEffort')(
      <A, E, R>(effect: Effect.Effect<A, E, R>) =>
        effect.pipe(
          Effect.catch((error) =>
            Effect.logWarning(
              'The D1 board directory projection is behind.',
              error,
            ),
          ),
          Effect.asVoid,
    ...
    const createBoard = Effect.fn('CloudflareBoardHandlers.CreateBoard')(
      function* (...args: Parameters<typeof boards.create>) {
    ...
    const duplicateBoard = Effect.fn('CloudflareBoardHandlers.DuplicateBoard')(
      function* (...args: Parameters<typeof boards.duplicate>) {
    ...
    const deleteBoard = Effect.fn('CloudflareBoardHandlers.DeleteBoard')(
    ...
    const commitBoard = Effect.fn('CloudflareBoardHandlers.CommitBoard')(
      function* (input: Parameters<typeof boards.commit>[0]) {
        ...
        yield* bestEffort(
          projection.upsert(
            summaryFromSnapshot(input.boardId, snapshot),
            snapshot.revision,
          ),
        );
