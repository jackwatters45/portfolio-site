...
} from '../client/media/spotify-embed';
import {
      ...
      .then((api) => {
        ...
        api.createController(
          mount,
          {
            uri: spotify.uri,
            width: '100%',
            height: String(SPOTIFY_AUDIO_CARD_HEIGHT),
          },
          (created) => {
            if (!active) {
              created.destroy();
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      if (
        isYouTubePlayingMessage(
          event,
          youtubeFrameRef.current?.contentWindow ?? null,
        )
      ) {
