...
export function BulkImageStager({
  ...
  const inspectEntries = useCallback(
    (created: ReadonlyArray<BulkImageEntry>) => {
      ...
      void runBoundedWorkers(
        created,
        (entry) => inspectImageFile(entry.file, signal),
        {
  ...
  const navigableIds = useMemo(
    () =>
      displayed
        .filter(
          (entry) => entry.status !== 'failed' && entry.status !== 'added',
        )
        .map((entry) => entry.id),
    [displayed],
  );
  ...
  const focusGridEntry = (currentId: string, key: string) => {
    ...
    const columns = Math.max(
      1,
      Math.floor(
        (dialogRef.current?.querySelector('.bulk-stager-grid')?.clientWidth ??
          160) / 154,
      ),
    );
    const nextIndex =
    ...
    dialogRef.current
      ?.querySelector<HTMLInputElement>(
        `input[data-bulk-entry="${CSS.escape(nextId)}"]`,
      )
      ?.focus();
  };
          ...
          {initialFailures.length > 0 && (
            <div className="bulk-stager-warning" role="status">
              <strong>
                ...
                {initialFailures.length === 1 ? 'entry' : 'entries'} could not
                be read.
              </strong>
              <ul>
