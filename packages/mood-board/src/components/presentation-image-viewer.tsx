    ...
    >
      ...
      <div ref={stageRef} className="presentation-image-viewer-stage">
        {item.kind === 'image' ? (
          ...
          <div
            className="presentation-item-viewer-frame"
            style={itemFrameStyle}
          >
            <div
              className={`presentation-item-viewer-surface board-item--${item.kind}`}
            >
              {item.kind === 'note' && (
      ...
      {item.kind === 'image' &&
        (item.annotationTitle || item.annotationDescription) && (
