...
export const minimumAudioCardHeight = (kind: AudioCardKind): number =>
  Match.value(kind).pipe(
    Match.when('spotify', () => MIN_SPOTIFY_AUDIO_CARD_HEIGHT),
    Match.when('youtube', () => MIN_YOUTUBE_AUDIO_CARD_HEIGHT),
    Match.orElse(() => MIN_NATIVE_AUDIO_CARD_HEIGHT),
  );

export const preferredAudioCardHeight = (
  kind: AudioCardKind,
  width: number,
): number =>
  Match.value(kind).pipe(
    Match.when('spotify', () => SPOTIFY_AUDIO_CARD_HEIGHT),
    Match.when('youtube', () =>
      Math.max(MIN_YOUTUBE_AUDIO_CARD_HEIGHT, Math.ceil((width * 9) / 16)),
    ),
    Match.orElse(() => NATIVE_AUDIO_CARD_HEIGHT),
  );
