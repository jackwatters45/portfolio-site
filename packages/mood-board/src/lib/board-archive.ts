...
import { BoardSchema } from './board-rpc';
import {
  hasValidMediaMagic,
...
const ManifestMediaSchema = Schema.Struct({
  ...
  Schema.makeFilter((entry) => {
    ...
    return entry.byteLength > mediaByteLimit(entry.kind)
      ? {
          path: ['byteLength'],
          issue: "Media entry exceeds its kind's size limit",
        }
      : undefined;
  }),
);
...
const decodeEmbeddedImage = (source: string): ArchiveMedia => {
  const match =
...
const preparePortableBoard = (board: Board) => {
  ...
  const embedded = new Map<MediaId, ArchiveMedia>();
...
export const boardMediaReferences = (board: Board): Map<MediaId, MediaKind> => {
...
export const mapArchiveEntries = async <T, R>(
...
export async function createPortableArchive(
  ...
  const references = [...boardMediaReferences(portable.board).entries()].sort(
  ...
  const downloaded = await mapArchiveEntries(
    references,
  ...
  if (manifestBytes.byteLength + total > MAX_ARCHIVE_BYTES)
    throw new Error('The archive expands beyond the safe size limit.');

  if (Option.isNone(decodeArchiveManifestEnvelope(manifest)))
    throw new Error('The board or camera is invalid.');
...
export type ArchiveReadOptions = { readonly signal?: AbortSignal };
...
export async function readPortableArchive(
  ...
  const references = boardMediaReferences(manifest.board);
