  ...
  (item: typeof BoardItemPayloadStruct.Type) => {
    ...
    if (item.kind === 'image') {
      ...
      if (
        ...
        return {
          path: ['src'],
          issue: 'Image items require a supported image source',
        };
      }
    }
    ...
    if (item.kind === 'spotify') {
      ...
      if (Option.isNone(decodeSpotifySourceUrl(item.src))) {
        return {
          path: ['src'],
          issue: 'Spotify cards require a canonical supported Spotify URL',
        };
      }
      ...
      if (Option.isNone(decodeYouTubeSourceUrl(item.src))) {
        return {
          path: ['src'],
          issue: 'YouTube cards require a canonical supported YouTube URL',
        };
      }
      ...
      if (
        ...
        return {
          path: ['src'],
          issue: 'Audio cards require a hosted HTTPS URL',
        };
      }
    }
...
export const makeBoardDocumentSchema = <Items extends Schema.Top>(
  items: Items,
) =>
  Schema.Struct({
