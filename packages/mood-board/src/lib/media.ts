...
export const MediaByteLengthSchema = PositiveIntegerSchema.pipe(
  Schema.brand('MediaByteLength'),
);

export type MediaByteLength = typeof MediaByteLengthSchema.Type;

export const MediaByteCountSchema = NonNegativeIntegerSchema.pipe(
  Schema.brand('MediaByteCount'),
);

export type MediaByteCount = typeof MediaByteCountSchema.Type;

export const MAX_IMAGE_UPLOAD_BYTES: MediaByteLength =
  MediaByteLengthSchema.make(12 * 1024 * 1024);

export const MAX_AUDIO_UPLOAD_BYTES: MediaByteLength =
  MediaByteLengthSchema.make(25 * 1024 * 1024);

export const MAX_MEDIA_UPLOAD_BYTES = MAX_AUDIO_UPLOAD_BYTES;

export const MediaByteOffsetSchema = NonNegativeIntegerSchema.pipe(
  Schema.brand('MediaByteOffset'),
);

export type MediaByteOffset = typeof MediaByteOffsetSchema.Type;

export const MediaTimestampSchema = NonNegativeIntegerSchema.pipe(
  Schema.brand('MediaTimestamp'),
);
...
export const MediaByteQuotaSchema = PositiveIntegerSchema.pipe(
  Schema.brand('MediaByteQuota'),
);
...
export const normalizeMediaKind = (
  value: string | null | undefined,
): MediaKind | null => Option.getOrNull(decodeMediaKind(value));
...
export const normalizeMediaMimeType = (
  ...
  return allowed.has(normalized)
    ? Option.getOrNull(decodeMediaMimeType(normalized))
    : null;
};
...
export const parseMediaRange = (
  ...
  return {
    start,
    end: MediaByteOffsetSchema.make(Math.min(requestedEnd, size - 1)),
  };
};
