  ...
  Schema.makeFilter((value) => {
    try {
      return new URL(value).origin === value
        ...
        : {
            path: [],
            issue: 'Authentication base URLs must be canonical origins',
          };
    } catch {
      return {
        path: [],
        issue: 'Authentication base URLs must be absolute origins',
      };
    }
  }),
...
export const validateAuthSecret = (
  secret: string,
  local: boolean,
): AuthSecret => {
...
export const validateAuthBaseURL = (
  value: string,
  local: boolean,
): AuthBaseUrl => {
  let url: URL;
...
const deliverMagicLinkEffect = Effect.fn('Auth.deliverMagicLink')(function* (
  ...
  if (!apiKey || !sender) {
    return yield* new MagicLinkDeliveryError({
      ...
      message:
        'RESEND_API_KEY and EMAIL_SENDER are required for magic-link delivery.',
    });
  }
