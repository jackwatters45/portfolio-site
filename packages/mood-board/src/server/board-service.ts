...
interface BoardOperations {
  ...
  readonly exists: (
    boardId: BoardId,
  ) => Effect.Effect<boolean, BoardBackendError>;
  readonly get: (
    boardId: BoardId,
  ) => Effect.Effect<BoardSnapshot | null, BoardBackendError>;
  readonly create: (
    boardId: BoardId,
    title: string,
  ...
  readonly commit: (
    input: CommitInput,
  ) => Effect.Effect<BoardChange, BoardBackendError>;
  readonly subscribe: (
    boardId: BoardId,
  ) => Stream.Stream<BoardEvent, BoardBackendError>;
}
...
>()('mood-board/BoardService') {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const channelFor = Effect.fn('BoardService.channelFor')(function* (
        boardId: BoardId,
      ) {
      ...
      yield* Effect.addFinalizer(() =>
        Effect.forEach(channels.values(), PubSub.shutdown, {
          discard: true,
        }).pipe(Effect.asVoid),
      );
      ...
      const exists = Effect.fn('BoardService.exists')(function* (
        boardId: BoardId,
      ) {
        return yield* recoverPersistence(
          mutex.withPermit(repo.exists(boardId)),
        );
      });

      const get = Effect.fn('BoardService.get')(function* (boardId: BoardId) {
        return yield* recoverPersistence(
          mutex.withPermit(repo.getSnapshot(boardId)),
        );
      });
      ...
      const commit = Effect.fn('BoardService.commit')(function* (
        input: CommitInput,
      ) {
      ...
      const subscribe = (
        ...
        Stream.unwrap(
          recoverPersistence(mutex.withPermit(openSubscription(boardId))),
        );
