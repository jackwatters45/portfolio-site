...
const STALE_TEMP_AGE_MS: MediaDurationMillis = MediaDurationMillisSchema.make(
  24 * 60 * 60 * 1_000,
);
...
const cleanupTemporary = Effect.fn('BunMediaObjectStore.cleanupTemporary')(
  (path: string) =>
    fsPromise(() => unlink(path)).pipe(
      Effect.catch((cause) =>
        isNotFound(cause)
          ? Effect.void
          : Effect.logWarning('Temporary media cleanup failed', cause),
      ),
    ),
);

const cleanupStaleTemps = Effect.fn('BunMediaObjectStore.cleanupStaleTemps')(
  function* (root: string) {
    yield* fsPromise(() => mkdir(root, { recursive: true }));
    const names = yield* fsPromise(() => readdir(root));
    const now = yield* Clock.currentTimeMillis;
    const cutoff = MediaTimestampSchema.make(
      Math.max(0, now - STALE_TEMP_AGE_MS),
    );
...
const deleteObject = Effect.fn('BunMediaObjectStore.delete')(function* (
  ...
  if (filename === null) {
    return yield* storageError(
      'delete',
      new Error('Invalid media storage key'),
    );
  }
  return yield* fsPromise(() => unlink(join(root, filename))).pipe(
    Effect.catch((cause) =>
      isNotFound(cause)
        ? Effect.void
        : Effect.fail(storageError('delete', cause)),
    ),
  );
});
...
const readObject = Effect.fn('BunMediaObjectStore.get')(function* (
  ...
  return yield* read.pipe(
    Effect.catch((cause) =>
      isNotFound(cause)
        ? Effect.succeed(null)
        : Effect.fail(storageError('read', cause)),
    ),
  );
});
