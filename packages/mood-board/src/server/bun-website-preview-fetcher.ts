...
const lookupAddresses = Effect.fn('BunWebsitePreviewFetcher.lookupAddresses')(
  (hostname: string) =>
    Effect.tryPromise({
        ...
        const request = httpsRequest(
          ...
          (response) => {
            const status = Option.getOrNull(
              decodeHttpStatusCode(response.statusCode),
            );
            if (status === null) {
              fail(
                new WebsiteFetchFailure({
            ...
            const rawContentType = headerValue(
...
const fetchPinned = Effect.fn('BunWebsitePreviewFetcher.fetchPinned')(
  function* (url: WebsiteUrl) {
...
const fetchOne = Effect.fn('BunWebsitePreviewFetcher.fetch')(
  (url: WebsiteUrl) =>
