...
const positiveInteger = (name: string, fallback: number): number =>
  Option.getOrNull(decodePositiveInteger(Number(process.env[name]))) ??
  fallback;
...
class ServerBoundaryError extends Schema.Error<ServerBoundaryError>(
  'ServerBoundaryError',
)({
...
const recoverServerBoundary = (error: ServerBoundaryError) =>
  Effect.logError(error).pipe(
    Effect.as(noStoreJson(500, 'The server is temporarily unavailable')),
  );
...
const servePublicMedia = Effect.fn('Server.ServePublicMedia')(function* () {
  ...
  if (!boardResponse.ok || !publicBoardReferencesMedia(publicBoard, mediaId))
    return notFound();
...
const requestMagicLink = Effect.fn('Server.RequestMagicLink')(function* () {
  const request = yield* HttpServerRequest.HttpServerRequest;
  const source = yield* HttpServerRequest.toWeb(request);
  const body = yield* tryServerBoundary('AuthRequestJson', () =>
    source.clone().json(),
  ).pipe(
    Effect.map(
      (value) => decodeMagicLinkEmail(value) ?? MISSING_MAGIC_LINK_EMAIL,
    ),
  ...
  const rateLimitTime = MagicLinkRateLimitTimestampSchema.make(
  ...
  return fromWebResponse(
    yield* tryServerBoundary('AuthHandler', () => auth.handler(source)),
  );
});
...
const AuthRoute = HttpRouter.use((router) =>
  Effect.gen(function* () {
    ...
    yield* router.add(
      ...
      requestMagicLink().pipe(
        Effect.catchTag('ServerBoundaryError', recoverAuthBoundary),
      ),
    );
    yield* router.add('*', '/api/auth/*', (request) =>
      forwardAuth(request).pipe(
        Effect.catchTag('ServerBoundaryError', recoverAuthBoundary),
      ),
    );
  }),
...
const PublicRoutes = HttpRouter.use((router) =>
  Effect.gen(function* () {
    ...
    yield* router.add(
      ...
      forwardPublicProfileRequest(),
    );
    yield* router.add(
      'GET',
      '/api/public/boards/:publicId',
      forwardPublicBoardRequest(),
    );
...
const SecurityHeaders = HttpRouter.middleware(
  (response) =>
    Effect.map(response, (value) =>
      HttpServerResponse.setHeaders(value, SECURITY_HEADERS),
    ),
  { global: true },
);
