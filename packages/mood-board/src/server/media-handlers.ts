  ...
  if (error instanceof MediaServiceError) {
    switch (error.reason) {
      ...
      case 'InvalidContent':
        return jsonError(
          400,
          'The upload is empty or does not match its declared media type.',
        );
      case 'TooLarge':
      ...
      case 'StorageQuota':
        return jsonError(
          507,
          'Managed media storage is full.',
          'media_storage_full',
        );
      case 'Timeout':
        return jsonError(
          408,
          'The media upload took too long.',
...
const recoverMediaUnavailable =
  (message: string) => (error: MediaServiceError) =>
  ...
  Effect.gen(function* () {
    ...
    const upload = Effect.fn('MediaHandlers.Upload')(function* () {
      ...
      return yield* uploadBody(
        ...
        declaredLength,
        reservation,
      ).pipe(Effect.ensuring(media.releaseReservation(reservation)));
    });
    ...
    yield* router.add(
      ...
      cleanup().pipe(
        Effect.catch(
          recoverMediaUnavailable('Media cleanup is temporarily unavailable.'),
        ),
      ),
    );
    ...
    yield* router.add(
      ...
      maintenance().pipe(
        Effect.catch(
          recoverMediaUnavailable(
            'Media maintenance is temporarily unavailable.',
          ),
        ),
      ),
    );
    ...
    yield* router.add(
      ...
      serve().pipe(
        ...
        Effect.catch(
          recoverMediaUnavailable('Media storage is temporarily unavailable.'),
        ),
      ),
    );
