...
export class PublishingError extends Schema.Error<PublishingError>(
  'PublishingError',
)({
...
interface PublishingServiceShape {
  ...
  readonly publish: (
    boardId: BoardId,
  ) => Effect.Effect<PublicId, PublishingError>;
  readonly unpublish: (
    boardId: BoardId,
  ) => Effect.Effect<boolean, PublishingError>;
  readonly getProfile: (
    handle: string,
  ) => Effect.Effect<PublicProfile | null, PublishingError>;
  readonly getBoard: (
    publicId: string,
  ) => Effect.Effect<PublicBoard | null, PublishingError>;
}
...
>()('mood-board/PublishingService') {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const configureProfile = Effect.fn('PublishingService.configureProfile')(
        function* (input: {
          readonly handle: string;
          readonly displayName: string;
          readonly bio: string;
      ...
      const publish = Effect.fn('PublishingService.publish')(function* (
        boardId: BoardId,
      ) {
      ...
      const unpublish = Effect.fn('PublishingService.unpublish')(function* (
        boardId: BoardId,
      ) {
        return yield* mutex
      ...
      const getProfile = Effect.fn('PublishingService.getProfile')(function* (
        rawHandle: string,
      ) {
      ...
      const getBoard = Effect.fn('PublishingService.getBoard')(function* (
        rawPublicId: string,
      ) {
