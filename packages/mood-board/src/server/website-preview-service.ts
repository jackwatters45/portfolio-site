...
interface WebsitePreviewServiceShape {
  ...
  readonly resolveXPost: (
    url: string,
  ) => Effect.Effect<XPostPreview, BoardBackendError>;
}
...
const logFetchFailure = (error: WebsiteFetchFailure) =>
  Effect.logWarning('Website preview fetch failed', error).pipe(
    Effect.as(null),
  );
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const xCache = new Map<
        XPostUrl,
        {
          readonly preview: XPostPreview;
          readonly expiresAt: XPreviewCacheTimestampMillis;
        }
      >();

      const resolveOne = Effect.fn('WebsitePreviewService.resolve')(function* (
        input: string,
      ) {
      ...
      const resolveXPostOne = Effect.fn('WebsitePreviewService.resolveXPost')(
        function* (input: string) {
          ...
          xCache.set(src, {
            preview,
            expiresAt: XPreviewCacheTimestampMillisSchema.make(
              insertionTime + ttl,
            ),
          });
          if (xCache.size > 100)
