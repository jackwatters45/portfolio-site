...
describe('AudioCard', () => {
  ...
  it('mounts YouTube automatically without autoplay or a redundant source link', () => {
    ...
    const markup = renderToStaticMarkup(
      <AudioCard item={youtube} coordinator={coordinator} />,
    );
    expect(markup).toContain('youtube-embed-mount');
    expect(markup).toContain('<iframe');
    expect(markup).toContain(
      'https://www.youtube-nocookie.com/embed/ryig6M3rZYU',
    );
  ...
  it('accepts playing events only from the mounted privacy-enhanced player', () => {
    ...
    expect(
      isYouTubePlayingMessage(
        { ...playing, origin: 'https://www.youtube.com' },
        source,
      ),
    ).toBe(false);
    expect(isYouTubePlayingMessage(playing, {} as MessageEventSource)).toBe(
      false,
    );
    expect(
      isYouTubePlayingMessage({ ...playing, data: 'not-json' }, source),
    ).toBe(false);
    expect(
      isYouTubePlayingMessage(
        {
  ...
  it('mounts Spotify automatically without a redundant source link', () => {
    ...
    const markup = renderToStaticMarkup(
      <AudioCard item={spotify} coordinator={coordinator} />,
    );
