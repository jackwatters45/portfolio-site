...
import {
  createBoardArchive,
  importBoardFile,
} from '../src/client/board/board-archive';
...
describe('portable mood-board archives', () => {
  it('deduplicates downloads and uploads, then remaps every reference', async () => {
    const download = vi.fn(async (_mediaId: MediaId, _kind: MediaKind) =>
      downloadedPng(),
    );
    ...
    expect(imported.board.items.map((item) => item.mediaId)).toEqual([
      targetId,
      targetId,
    ]);
    expect(imported.board.items.every((item) => item.src === undefined)).toBe(
      true,
    );
  ...
  it('round-trips an archive whose only managed reference is the background', async () => {
    ...
    const archive = await createBoardArchive(backgroundOnly, camera, {
      download,
    });
    const upload = vi.fn(async (blob: Blob, kind: MediaKind) =>
      uploadedMedia(blob, kind, targetId),
  ...
  it('rejects missing or MIME-mismatched media before uploading', async () => {
    ...
    const upload = vi.fn();
    await expect(
      importBoardFile(new File([zipSync(unpacked)], 'mismatch.moodboard'), {
        upload,
      }),
    ...
    await expect(
      importBoardFile(new File([zipSync(unpacked)], 'missing.moodboard'), {
        upload,
      }),
  ...
  it('rejects same-size media corruption before uploading', async () => {
    ...
    const upload = vi.fn();
    await expect(
      importBoardFile(new File([zipSync(unpacked)], 'corrupt.moodboard'), {
        upload,
      }),
  ...
  it('converts and deduplicates embedded images in a mixed archive', async () => {
    ...
    const upload = vi.fn(async (blob: Blob, kind: MediaKind) => {
      ...
      const mediaId =
        uploadCount === 1
          ? targetId
          : MediaIdSchema.make('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
    ...
    expect(imported.board.items[2]?.mediaId).toBe(
      imported.board.items[3]?.mediaId,
    );
  });
    ...
    await expect(
      importBoardFile(await asFile(archive), {
        upload: async (blob, kind) => {
          ...
          completed += 1;
          const mediaId = MediaIdSchema.make(
            started.toString(16).padStart(32, 'a'),
          );
          return uploadedMedia(blob, kind, mediaId);
        },
      }),
