...
describe('X card import validation', () => {
  ...
  it('canonicalizes legacy sources and sanitizes snapshot text', () => {
    const item = normalizeImportedBoard(
      xBoard(
        'https://twitter.com/sheherenow_/status/2082226100764369045?ref=x',
      ),
    ).board.items[0];
  ...
  it('rejects invalid settings and metadata on other item kinds', () => {
    expect(() =>
      normalizeImportedBoard(xBoard('https://example.com/status/123')),
    ).toThrow();
    expect(() =>
...
describe('swatch import validation', () => {
  ...
  it('keeps generic text and labels only on their owning item kinds', () => {
    ...
    expect(imported.board.items[0]?.text).toBeUndefined();
    expect(imported.board.items[0]?.label).toBeUndefined();
    expect(imported.board.items[1]).toMatchObject({
      kind: 'swatch',
      label: 'Palette',
    });
    expect(imported.board.items[1]?.text).toBeUndefined();
    expect(imported.board.items[2]).toMatchObject({
      kind: 'note',
      text: 'Keep this note',
    });
  ...
  it('normalizes an optional board background', () => {
    ...
    expect(
      normalizeImportedBoard(makeBoard('#12ABEF')).board.background,
    ).toBeUndefined();
  });
  ...
  it('normalizes image links and preserves older unlinked images', () => {
    expect(
      normalizeImportedBoard(imageBoard(' HTTP://SHOP.EXAMPLE/chair ')).board
        .items[0]?.href,
    ).toBe('http://shop.example/chair');
    const withoutLink = imageBoard(undefined);
    delete withoutLink.items[0]?.href;
    expect(
      normalizeImportedBoard(withoutLink).board.items[0]?.href,
    ).toBeUndefined();
  });
  ...
  it('normalizes image annotations and rejects them on other item kinds', () => {
    ...
    expect(() =>
      normalizeImportedBoard({
        ...makeBoard('#12ABEF'),
        items: [
          { ...makeBoard('#12ABEF').items[0], annotationTitle: 'Smuggled' },
        ],
      }),
  ...
  it('rejects unsafe links and links on non-image items', () => {
    expect(() =>
      normalizeImportedBoard(imageBoard('javascript:alert(1)')),
    ).toThrow('invalid external link');
    expect(() =>
      normalizeImportedBoard({
        ...makeBoard('#12ABEF'),
        items: [
  ...
  it('normalizes Spotify and hosted audio sources', () => {
    expect(
      normalizeImportedBoard(
        audioBoard(
          'spotify',
          'https://open.spotify.com/embed/track/4uLU6hMCjMI75M1A2tKUQC?si=x',
        ),
      ).board.items[0]?.src,
    ).toBe('https://open.spotify.com/track/4uLU6hMCjMI75M1A2tKUQC');
    expect(
      normalizeImportedBoard(
        audioBoard('youtube', 'https://youtu.be/ryig6M3rZYU?si=tracking'),
      ).board.items[0]?.src,
    ).toBe('https://www.youtube.com/watch?v=ryig6M3rZYU');
    expect(
      normalizeImportedBoard(
        audioBoard('audio', 'https://media.example/night-train.mp3'),
      ).board.items[0]?.kind,
    ).toBe('audio');
  });
  ...
  it('only accepts managed IDs through the portable archive path', () => {
    ...
    expect(() => normalizeImportedBoard(value)).toThrow(
      /portable \.moodboard archive/i,
    );
    expect(() =>
      normalizeImportedBoard({
        ...makeBoard('#12ABEF'),
      ...
      normalizeImportedBoard(
        {
          ...value,
          items: [
            { ...value.items[0], src: 'https://images.example/chair.jpg' },
          ],
        },
        { allowManagedMedia: true },
      ),
  ...
  it('rejects browser-local and malformed audio sources', () => {
    expect(() =>
      normalizeImportedBoard(
        audioBoard('audio', 'data:audio/mpeg;base64,AAAA'),
      ),
    ).toThrow('hosted HTTPS source');
    expect(() =>
      normalizeImportedBoard(
        audioBoard('audio', 'blob:https://example.com/id'),
      ),
    ).toThrow('hosted HTTPS source');
    expect(() =>
      normalizeImportedBoard(
        audioBoard('spotify', 'https://evil.example/track/id'),
      ),
    ).toThrow('invalid source');
    expect(() =>
      normalizeImportedBoard(
        audioBoard('youtube', 'https://www.youtube.com/watch?v=too-short'),
      ),
    ).toThrow('invalid source');
  });
  ...
  it('preserves normalized website preview snapshots', () => {
    const item = normalizeImportedBoard(
      websiteBoard(' https://EXAMPLE.com/story#top '),
    ).board.items[0];
    expect(item).toMatchObject({
      kind: 'website',
  ...
  it('rejects incomplete, private, and cross-kind website metadata', () => {
    expect(() =>
      normalizeImportedBoard(websiteBoard('https://127.0.0.1/admin')),
