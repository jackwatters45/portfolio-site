...
describe('custom hex colors', () => {
  ...
  it('chooses accessible field text and selection colors', () => {
    ...
    for (const background of [
      ...
      expect(
        contrastRatio(background, colors.foreground),
      ).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(background, colors.muted)).toBeGreaterThanOrEqual(
        4.5,
      );
      expect(
        contrastRatio(background, colors.selection),
