...
describe('bulk assisted layouts', () => {
  ...
  it('rejects extreme ratios instead of silently cropping them', () => {
    expect(() =>
      layoutBulkImages([{ id: 'too-wide', width: 900, height: 100 }], 'loose'),
    ).toThrow('between 1:8 and 8:1');
    expect(() =>
      layoutBulkImages(
        [{ id: 'too-tall', width: 100, height: 900 }],
        'masonry',
      ),
    ).toThrow('between 1:8 and 8:1');
  });
  ...
  it('centers a free group and walks outward around rotated obstacles', () => {
    ...
    const moved = placeLayoutWithoutOverlap(layout, { x: 500, y: 400 }, [
      obstacle,
    ]);
    expect(moved.bounds).not.toEqual(centered.bounds);
    expect(boundsOverlap(moved.bounds, rotatedItemBounds(obstacle), 32)).toBe(
      false,
    );
    expect(
      placeLayoutWithoutOverlap(layout, { x: 500, y: 400 }, [obstacle]),
    ).toEqual(moved);
  });
  ...
  it('avoids clustered existing items without overlapping the imported group', () => {
    ...
    expect(
      obstacles.some((item) =>
        boundsOverlap(placed.bounds, rotatedItemBounds(item), 32),
      ),
    ).toBe(false);
