...
describe('board sync model', () => {
  ...
  it('diffs and replays managed media IDs without inventing a URL', () => {
    ...
    const initial = board();
    const mutation = diffBoards(
      initial,
      board({ items: [managed], updatedAt: 2 }),
    )[0];
  ...
  it('preserves image annotations and detects annotation-only edits', () => {
    ...
    const mutation = diffBoards(
      initial,
      board({ items: [edited], updatedAt: 2 }),
    )[0];
  ...
  it('preserves website preview fields and detects metadata-only refreshes', () => {
    ...
    const mutation = diffBoards(
      initial,
      board({ items: [refreshed], updatedAt: 2 }),
    )[0];
  ...
  it('preserves X settings and detects snapshot-only refreshes', () => {
    ...
    const refreshed = makeItem({
      ...x,
      xPostText: 'Updated cooking inspiration',
    });
    const mutation = diffBoards(
      initial,
      board({ items: [refreshed], updatedAt: 2 }),
  ...
  it('diffs, applies, and resets a board background independently', () => {
    ...
    expect(
      applyBoardMutation(initial, diffBoards(initial, colored)[0]),
    ).toMatchObject({
      background: '#242728',
    });
  ...
  it('diffs, applies, and clears managed background media independently', () => {
    ...
    expect(
      applyBoardMutation(initial, diffBoards(initial, managed)[0]),
    ).toMatchObject({
      background: '#242728',
      backgroundMediaId: mediaId,
    });
    expect(
      diffBoards(managed, board({ background: '#242728', updatedAt: 3 })),
    ).toEqual([{ backgroundMediaId: null, upserts: [], deletes: [] }]);
  });
