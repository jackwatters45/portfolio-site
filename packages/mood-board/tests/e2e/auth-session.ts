...
export const createBunTestSession = async (config: {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(`Magic-link setup failed: ${response.status}`);
  } finally {
  ...
  const magicLink =
    message === undefined ? undefined : /\burl=(\S+)/.exec(message)?.[1];
  if (magicLink === undefined)
    throw new Error('The local magic link was not captured.');
