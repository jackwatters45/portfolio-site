...
const startServer = async () => {
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    if (server.exitCode !== null) {
      throw new Error(
        `Server exited during startup: ${await new Response(server.stderr).text()}`,
      );
    }
    try {
...
const runPublisher = async (
  ...
  const child = Bun.spawn(
    [
      'bun',
      'scripts/bootstrap-publisher.ts',
      ...args,
      '--account-id',
  ...
  if (!isRecord(parsed))
    throw new Error('Publisher CLI returned invalid JSON.');
  return parsed;
};
...
try {
  ...
  const securityPolicy =
    (await fetch(`http://127.0.0.1:${port}/health`)).headers.get(
  ...
  assert.equal(
    guestBoardPage.headers.get('location'),
    '/login?returnTo=%2Fboards%2Fdefault',
  );
  ...
  const exists = await writer.runPromise(
    TestClient.use((client) =>
      client.BoardExists({ boardId: DEFAULT_BOARD_ID }),
    ),
  );
  ...
  assert.deepEqual(
    await otherAccount.runPromise(
      TestClient.use((client) => client.ListBoards()),
    ),
    [],
  );
  await otherAccount.runPromise(
  ...
  assert.equal(
    (
      await writer.runPromise(TestClient.use((client) => client.ListBoards()))
    )[0]?.title,
    'Realtime test',
  );
  const privateMaintenance = await fetch(
    `http://127.0.0.1:${port}/_internal/media/maintenance`,
    {
      method: 'POST',
  ...
  assert.equal(
    limitedUploadResponse.headers.get('cache-control'),
    'private, no-store',
  );
  ...
  const managedMediaId = Schema.decodeUnknownSync(MediaIdSchema)(
    mediaUpload.mediaId,
  );
      ...
      client.CommitBoard({
        ...
        upserts: [
          ...
          },
          {
            ...
            websiteImageUrl: WebsiteImageUrlSchema.make(
              'https://cdn.example.com/story.jpg',
            ),
            websiteTitle: WebsiteTitleSchema.make('A collected room'),
            websiteDescription: WebsiteDescriptionSchema.make(
              'Light, stone, and quiet objects.',
            ),
  ...
  const invalidResponse = await fetch(
    `http://127.0.0.1:${port}/api/public/boards/default`,
  );
  const malformedResponse = await fetch(
    `http://127.0.0.1:${port}/api/public/boards/default/edit`,
  );
  ...
  const publicBoardResponse = await fetch(
    `http://127.0.0.1:${port}/api/public/boards/${publicId}`,
  );
  ...
  const secondBoardId = BoardIdSchema.make(
  ...
  const listed = await writer.runPromise(
    TestClient.use((client) => client.ListBoards()),
  );
  assert.deepEqual(
    ...
    TestClient.use((client) =>
      client.SubscribeBoard({ boardId: secondBoardId }).pipe(
        ...
        Stream.runForEach((event) =>
          Effect.sync(() => duplicateEvents.push(event)),
        ),
      ),
    ),
  );
  ...
  assert.equal(
    duplicateEvents[0]?._tag === 'Snapshot' &&
      duplicateEvents[0].board.items[0]?.text,
    'persisted and broadcast',
  );
  assert.equal(
    duplicateEvents[0]?._tag === 'Snapshot' &&
      duplicateEvents[0].board.background,
    '#242728',
  );
  assert.equal(
    duplicateEvents[0]?._tag === 'Snapshot' &&
      duplicateEvents[0].board.items.find((item) => item.id === 'image-1')
        ?.href,
    'https://shop.example/chair',
  );
  assert.equal(
    duplicateEvents[0]?._tag === 'Snapshot' &&
      duplicateEvents[0].board.items.find((item) => item.id === 'spotify-1')
        ?.kind,
    'spotify',
  );
  assert.equal(
    duplicateEvents[0]?._tag === 'Snapshot' &&
      duplicateEvents[0].board.items.find((item) => item.id === 'youtube-1')
        ?.src,
    'https://www.youtube.com/watch?v=ryig6M3rZYU',
  );
  ...
  assert.equal(
    snapshot.board.items.find((item) => item.id === 'image-1')?.mediaId,
    managedMediaId,
  );
  ...
  assert.equal(
    persistedMediaResponse.headers.get('content-range'),
    'bytes 5-7/8',
  );
  ...
  const persistedPublicResponse = await fetch(
    `${origin}/api/public/boards/${publicId}`,
  );
  ...
  await Promise.all(
    runtimes.map((runtime) => runtime.dispose().catch(() => undefined)),
  );
