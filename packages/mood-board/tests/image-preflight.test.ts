...
const box = (type: string, data: Uint8Array) =>
  concat(u32(data.length + 8), ascii(type), data);
...
const heic = (width: number, height: number, brand = 'heic') => {
  const ftyp = box(
    'ftyp',
    concat(ascii(brand), u32(0), ascii('mif1'), ascii(brand)),
  );
...
describe('image upload preflight', () => {
  it('reads supported dimensions from headers without raster decoding', () => {
    ...
    expect(preflightImageBytes(gif(640, 480))).toEqual({
      format: 'gif',
      width: 640,
      height: 480,
    });
  ...
  it('accepts typical 48 MP iPhone dimensions and exact safety boundaries', () => {
    expect(
      validateImageLimits(
        { type: 'heic', width: 8064, height: 6048 },
        MAX_SOURCE_IMAGE_BYTES,
      ),
    ).toEqual({ format: 'heic', width: 8064, height: 6048 });
    expect(
      validateImageLimits({ type: 'png', width: 10_000, height: 6_400 }, 1),
    ).toEqual({
      format: 'png',
      width: 10_000,
      height: 6_400,
  ...
  it('rejects excessive axes, decoded pixels, aspect ratios, and bytes', () => {
    expect(() =>
      validateImageLimits({ type: 'jpeg', width: 10_001, height: 100 }, 1),
    ).toThrow(/10,000 pixels/i);
    expect(() =>
      validateImageLimits({ type: 'png', width: 8_001, height: 8_000 }, 1),
    ).toThrow(/64 megapixels/i);
    expect(() =>
      validateImageLimits({ type: 'heic', width: 8_200, height: 6_100 }, 1),
    ).toThrow(/50 megapixels/i);
    expect(() =>
      validateImageLimits({ type: 'webp', width: 8000, height: 999 }, 1),
    ).toThrow(/wide or tall/i);
    expect(() =>
      validateImageLimits(
  ...
  it('uses the signature rather than trusting HEIC extensions or MIME', async () => {
    ...
    const spoofed = new File([ascii('not a heic image')], 'spoof.HEIC', {
      type: 'image/heic',
    });
    await expect(preflightImageFile(spoofed)).rejects.toThrow(
      /not a supported or valid image/i,
    );
  });
  ...
  it('rejects valid but unsupported AVIF and truncated headers', () => {
    ...
    expect(() =>
      preflightImageBytes(new Uint8Array([0x89, 0x50, 0x4e])),
    ).toThrow(/supported or valid/i);
  });
});
