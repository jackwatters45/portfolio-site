...
describe('audio sources', () => {
  it('canonicalizes supported Spotify URLs', () => {
    for (const type of [
      ...
      expect(
        parseSpotifySource(
          `https://open.spotify.com/${type}/${id}?si=tracking`,
        ),
      ).toMatchObject({
        kind: 'spotify',
    ...
    expect(
      parseSpotifySource(
        `https://open.spotify.com/intl-de/embed/track/${id}?utm_source=generator`,
      )?.src,
  ...
  it('accepts hosted HTTPS audio and rejects browser-local or insecure sources', () => {
    ...
    ).toBe('https://media.example/field-recording.mp3?token=x');
    for (const value of [
  ...
  it('classifies sources and only auto-detects obvious pasted audio URLs', () => {
    expect(parseAudioSource(`https://open.spotify.com/track/${id}`)?.kind).toBe(
      'spotify',
    );
    expect(parseAudioSource(youtubeFixture)?.kind).toBe('youtube');
    expect(parseAudioSource('https://media.example/audio.ogg')?.kind).toBe(
      'audio',
    );
    expect(isLikelyAudioUrl(youtubeFixture)).toBe(true);
    expect(isLikelyAudioUrl('https://media.example/audio.m4a?download=1')).toBe(
      true,
    );
