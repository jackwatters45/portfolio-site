...
describe('board RPC schemas', () => {
  ...
  it('rejects unsafe board ids', () => {
    const exit = Effect.runSyncExit(
      Schema.decodeUnknownEffect(BoardIdSchema)('../default'),
    );
  ...
  it('requires note text and swatch color', () => {
    expect(
      Exit.isFailure(
        Effect.runSyncExit(decodeItem({ ...validItem, text: undefined })),
      ),
    ).toBe(true);
    expect(
      Exit.isFailure(
  ...
  it('accepts custom six-digit hex colors and rejects invalid values', () => {
    ...
    expect(
      Exit.isFailure(
        Effect.runSyncExit(decodeItem({ ...swatch, color: '#12ABEZ' })),
      ),
    ).toBe(true);
  });
  ...
  it('allows normalized links only on image items', () => {
    ...
    expect(
      Exit.isFailure(
        Effect.runSyncExit(
          decodeItem({ ...image, href: 'https://shop.example' }),
        ),
      ),
    ).toBe(true);
    expect(
      Exit.isFailure(
        Effect.runSyncExit(
          decodeItem({ ...image, href: 'javascript:alert(1)' }),
        ),
      ),
    ).toBe(true);
    expect(
      Exit.isFailure(
        Effect.runSyncExit(
          decodeItem({ ...validItem, href: 'https://shop.example/chair' }),
        ),
      ),
    ).toBe(true);
    expect(
      Exit.isFailure(
        Effect.runSyncExit(
          decodeItem({ ...validItem, annotationTitle: 'Smuggled details' }),
        ),
      ),
    ).toBe(true);
    expect(
      Exit.isFailure(
        Effect.runSyncExit(
          decodeItem({ ...image, annotationTitle: '  Padded title  ' }),
        ),
      ),
    ).toBe(true);
  });
  ...
  it('validates canonical Spotify and hosted audio item sources', () => {
    ...
    for (const invalid of [
      ...
      { ...validItem, src: 'data:audio/mpeg;base64,AAAA' },
      {
        ...validItem,
        kind: 'swatch' as const,
        src: 'blob:https://example.com/audio',
      },
      {
        ...validItem,
        kind: 'image' as const,
      ...
      expect(Exit.isFailure(Effect.runSyncExit(decodeItem(invalid)))).toBe(
        true,
      );
  });
  ...
  it('validates canonical X cards and bounded snapshots', () => {
    ...
    for (const invalid of [
      {
        ...x,
        src: 'https://twitter.com/sheherenow_/status/2082226100764369045',
      },
  ...
  it('rejects invalid geometry', () => {
    const exit = Effect.runSyncExit(
      decodeItem({ ...validItem, width: Number.POSITIVE_INFINITY }),
    );
    expect(Exit.isFailure(exit)).toBe(true);
  });
  ...
  it('validates optional board backgrounds and explicit reset changes', () => {
    ...
    expect(
      Exit.isFailure(
        Effect.runSyncExit(decodeBoard({ ...base, background: 'charcoal' })),
      ),
    ).toBe(true);
    expect(
      Exit.isFailure(
        Effect.runSyncExit(
          decodeBoard({ ...base, backgroundMediaId: 'not-media' }),
        ),
      ),
    ).toBe(true);
