...
describe('BoardService', () => {
  ...
  it.effect('persists, duplicates, and removes image links', () =>
    Effect.gen(function* () {
      ...
      expect((yield* service.get(boardA))?.board.items[0]?.href).toBe(
      ...
      expect(
        (yield* service.get(boardA))?.board.items[0]?.href,
      ).toBeUndefined();
    }).pipe(Effect.provide(TestLayer)),
  );
  ...
  it.effect(
    'persists and duplicates Spotify, YouTube, and hosted audio cards',
    () =>
      Effect.gen(function* () {
        const service = yield* BoardService;
  ...
  it.effect('canonicalizes YouTube sources in direct service calls', () =>
    Effect.gen(function* () {
      ...
      expect(change.upserts[0]?.src).toBe(
    ...
    Effect.gen(function* () {
      ...
      const mediaError = yield* service
        .commit({
          ...
          clientId: ClientIdSchema.make('background-client'),
          mutationId: MutationIdSchema.make('background-media-hash'),
          backgroundMediaId: MediaIdSchema.make(
  ...
  it.effect('lists, duplicates, and isolates boards', () =>
    Effect.gen(function* () {
      ...
      const duplicate = yield* service.duplicate(
        boardA,
        boardB,
        'Trip ideas — copy',
      );
  ...
  it.effect(
    'rejects overlapping upserts and deletes without changing the snapshot',
    () =>
      Effect.gen(function* () {
        const service = yield* BoardService;
        yield* service.create(DEFAULT_BOARD_ID, 'Test');
  ...
  it.effect(
    'publishes deletion to active subscribers and preserves another board',
    () =>
      Effect.gen(function* () {
        const service = yield* BoardService;
        yield* service.create(DEFAULT_BOARD_ID, 'Home');
        ...
        expect((yield* service.list()).map((entry) => entry.id)).toEqual([
          DEFAULT_BOARD_ID,
        ]);
        yield* Fiber.interrupt(fiber);
      }).pipe(Effect.provide(TestLayer)),
    ...
    Effect.gen(function* () {
      ...
      for (let index = 0; index < 99; index += 1) {
        yield* service.create(
          BoardIdSchema.make(crypto.randomUUID()),
          `Board ${index}`,
        );
      }
