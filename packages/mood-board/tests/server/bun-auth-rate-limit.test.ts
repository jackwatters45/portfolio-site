...
import {
  decodeMagicLinkEmail,
  type MagicLinkRateLimitEmail,
...
const rateLimitTimestamp = (value: number) =>
  MagicLinkRateLimitTimestampSchema.make(value);
  ...
  it('isolates IP and normalized recipient buckets', () => {
    ...
    for (let index = 0; index < 5; index += 1) {
      expect(
        limiter.allow(
          '192.0.2.1',
          rateLimitEmail('Reader@Example.com'),
          rateLimitTimestamp(index),
        ),
      ).toBe(true);
    }
    expect(
      limiter.allow(
        '192.0.2.1',
        rateLimitEmail('reader@example.com'),
        rateLimitTimestamp(5),
      ),
    ).toBe(false);
    expect(
      limiter.allow(
        '192.0.2.2',
        rateLimitEmail('other@example.com'),
        rateLimitTimestamp(5),
      ),
    ).toBe(true);
  });
