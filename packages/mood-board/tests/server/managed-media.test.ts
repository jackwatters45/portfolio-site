...
const mediaQuotaClientId = (value: string) =>
  MediaQuotaClientIdSchema.make(value);
...
const managedImage = (
  mediaId: MediaId,
  id = ItemIdSchema.make('managed-image'),
) => ({
  id,
  kind: 'image' as const,
  x: 0,
...
describe('managed media persistence', () => {
  it.effect(
    'validates references, duplicates them, and claims only unreferenced assets',
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    'starts the cleanup grace period when the last reference is removed',
    () =>
      Effect.gen(function* () {
        const boards = yield* BoardService;
        const media = yield* MediaService;
  ...
  it.effect(
    'keeps failed upload objects discoverable until cleanup can retry',
    () => {
  ...
  it.effect(
    'keeps reservations atomic and reports distinct quota failures',
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const concurrent = yield* Effect.all(
          [
            Effect.result(
              media.reserveUpload(
                mediaQuotaClientId('other-client'),
                mediaByteLength(6),
              ),
            ),
            Effect.result(
        ...
        expect(
          concurrent.filter((result) => result._tag === 'Success'),
        ).toHaveLength(1);
        expect(
          concurrent.filter(
            (result) =>
              result._tag === 'Failure' &&
              result.failure.reason === 'StorageQuota',
          ),
        ).toHaveLength(1);
        for (const result of concurrent) {
          if (result._tag === 'Success')
            yield* media.releaseReservation(result.success);
        }
      ...
      `;
        ...
        const storageFailure = yield* media
          .reserveUpload(
            mediaQuotaClientId('storage-client'),
            mediaByteLength(3),
          )
          .pipe(Effect.flip);
  ...
  it.effect(
    'rolls back objects and pending metadata when a reservation is invalid',
    () => {
      const store = controlledStore();
      store.allowDeletes();
  ...
  it.effect(
    'reports when enough rolling byte events, rather than only the oldest, expire',
    () => {
      const layer = MediaService.layerWith(
        mediaQuotaLimits({
          requestsPerHour: 10,
  ...
  it.effect(
    'allows the default 150-file bulk workflow and enforces request rate state',
    () => {
