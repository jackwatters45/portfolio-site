...
describe('database migrations', () => {
  it.effect(
    'upgrades a populated v4 item table without losing data or constraints',
    () =>
      Effect.gen(function* () {
      ...
      `);
        yield* sql.unsafe(
          'CREATE INDEX items_board_order ON items (board_id, order_index)',
        );
        yield* sql.unsafe(`
      ...
      `)
        ...
        expect(
          indexes.some((index) => index.name === 'items_board_order'),
        ).toBe(true);
        expect(indexes.some((index) => index.name === 'items_media_id')).toBe(
          true,
        );
      ...
      `;
        ...
        const remaining =
          yield* sql<CountRow>`SELECT COUNT(*) AS count FROM items`;
        const publications =
          yield* sql<CountRow>`SELECT COUNT(*) AS count FROM board_publications`;
  ...
  it.effect(
    'upgrades a populated v6 audio table to website cards without data loss',
    () =>
      Effect.gen(function* () {
        const sql = yield* SqlClient.SqlClient;
        yield* sql.unsafe('PRAGMA foreign_keys = ON');
  ...
  it.effect('adds an indexed ready-image board background reference', () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      expect(
        yield* SqliteMigrator.run({ loader: migrationLoader }),
      ).toContainEqual([11, 'board_background_media']);
      const indexes = yield* sql.unsafe<IndexRow>('PRAGMA index_list(boards)');
      expect(
        indexes.some((index) => index.name === 'boards_background_media_id'),
      ).toBe(true);
  ...
  it.effect(
    'upgrades populated v9 media state and backfills authoritative bytes',
    () =>
      Effect.gen(function* () {
        const sql = yield* SqlClient.SqlClient;
        yield* sql.unsafe(`
