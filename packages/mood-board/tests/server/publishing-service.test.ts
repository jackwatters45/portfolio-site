...
const backgroundMediaId = MediaIdSchema.make(
...
describe('PublishingService', () => {
  it.effect(
    'denies private and unknown boards before any public happy path',
    () =>
      Effect.gen(function* () {
        const { publishing } = yield* prepare;
        expect(
          yield* publishing.getBoard('0123456789abcdef0123456789abcdef'),
        ).toBeNull();
  ...
  it.effect(
    'publishes explicitly, preserves the public id, and omits internal ids',
    () =>
      Effect.gen(function* () {
        const { publishing } = yield* prepare;
  ...
  it.effect(
    'rejects invalid profile fields and publishing without a profile',
    () =>
      Effect.gen(function* () {
        const { publishing } = yield* prepare;
        const invalid = yield* publishing
          ...
          .pipe(Effect.exit);
        const missingProfile = yield* publishing
          .publish(boardId)
          .pipe(Effect.exit);
