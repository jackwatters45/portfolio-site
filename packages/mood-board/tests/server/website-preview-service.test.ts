...
const redirectLocation = (value: string) =>
  WebsiteRedirectLocationSchema.make(value);
...
describe('WebsitePreviewService', () => {
  it.effect(
    'resolves a sanitized X snapshot and degrades to a durable source',
    () =>
      Effect.gen(function* () {
    ...
    Effect.gen(function* () {
      const preview = yield* run('https://example.com/start', {
        ...
        'https://example.com/story': {
          ...
          body: new TextEncoder().encode(
            '<meta property="og:title" content="A quiet story">',
          ),
        },
      });
  ...
  it.effect(
    'rejects private redirects and falls back for non-HTML responses',
    () =>
      Effect.gen(function* () {
