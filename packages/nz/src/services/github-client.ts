...
export class GitHubClient extends Context.Service<GitHubClient>()(
  'nz/GitHubClient',
  {
    make: (token: Redacted.Redacted<string>) =>
      Effect.gen(function* () {
            ...
            return response.pipe(
              ...
              Effect.filterOrFail(
                ...
                () =>
                  new CommentError({
                    status: 429,
                    message: 'Comments are busy. Please try again in a minute.',
                  }),
              ),
              Effect.flatMap(
        ...
        const listAll = Effect.fn('GitHubClient.listAll')(
          <A>(schema: Schema.Codec<A, unknown>, path: string) =>
            Stream.paginate(1, (page) =>
              Effect.gen(function* () {
                yield* Effect.succeed(page).pipe(
                  Effect.filterOrFail(
                    (value) => value <= 20,
                    () =>
                      new CommentError({
                ...
                const next = Option.some(page + 1).pipe(
                  Option.filter(
                    () =>
        ...
        return { owner: OWNER, client, listAll };
      }),
  },
) {
